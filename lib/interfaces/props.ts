...
export type SelectPropsResolver = {
  [K in Exclude<
    keyof SelectProps,
    keyof IPropsFieldBase | keyof IBaseFieldProps | 'displayEmpty' | 'placeholder'
  >]?: SelectProps[K];
};
...
export type DateTimePropsResolver = {
  [K in Exclude<
    keyof DateTimePickerModalProps,
    keyof IPropsFieldBase | keyof IBaseFieldProps
  >]?: DateTimePickerModalProps[K];
};
...
//   [K in Exclude<
//     KnownKeys<AutoCompleteProps<TextFieldProps>>,
//     keyof IPropsFieldBase | keyof IBaseFieldProps | keyof IStyledProps | 'isDisabled'
